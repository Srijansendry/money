...
export default function MissionControlPlanner() {
  ...
  const [viewMode, setViewMode] = useState<"MISSION_CONTROL" | "CALENDAR" | "ACADEMICS" | "PROBLEMS" | "LEETCODE" | "SIGMA9">("MISSION_CONTROL");
  ...
  const [editChapterOverview, setEditChapterOverview] = useState("");
  const [editChapterNotes, setEditChapterNotes] = useState("");
  const [chapterOverrides, setChapterOverrides] = useState<Record<string, { name: string; overview: string; notes?: string }>>(() => {
    try { return JSON.parse(localStorage.getItem("chapter-overrides") || "{}"); } catch { return {}; }
  });
  ...
  // Quick note inline panel
  const [activeProblemNote, setActiveProblemNote] = useState<string | null>(null);
  const [quickNoteFields, setQuickNoteFields] = useState<Record<string, { title: string; difficulty: string; notesTxt: string; code: string }>>({});
  const [quickNoteSaving, setQuickNoteSaving] = useState(false);
  const quickNoteFileRef = useRef<Record<string, File | null>>({});

  const patchQuickNote = (num: string, patch: Partial<{ title: string; difficulty: string; notesTxt: string; code: string }>) => {
    setQuickNoteFields((prev) => {
      const base = prev[num] ?? { title: `LC-${num}`, difficulty: "Medium", notesTxt: "", code: "" };
      return { ...prev, [num]: { ...base, ...patch } };
    });
  };

  const openQuickNote = (num: string, existing?: any) => {
    if (activeProblemNote === num) { setActiveProblemNote(null); return; }
    if (existing && !quickNoteFields[num]) {
      setQuickNoteFields((prev) => ({
        ...prev,
        [num]: { title: existing.title ?? `LC-${num}`, difficulty: existing.difficulty ?? "Medium", notesTxt: existing.notes ?? "", code: existing.codeSolution ?? "" },
      }));
    } else if (!quickNoteFields[num]) {
      setQuickNoteFields((prev) => ({ ...prev, [num]: { title: `LC-${num}`, difficulty: "Medium", notesTxt: "", code: "" } }));
    }
    setActiveProblemNote(num);
  };

  const saveQuickNote = async (num: string, existingNote?: any) => {
    const fields = quickNoteFields[num] ?? { title: `LC-${num}`, difficulty: "Medium", notesTxt: "", code: "" };
    const file = quickNoteFileRef.current[num] ?? null;
    const fd = new FormData();
    fd.append("problemNumber", num);
    fd.append("title", fields.title || `LC-${num}`);
    fd.append("difficulty", fields.difficulty || "Medium");
    fd.append("dateSolved", new Date().toISOString().slice(0, 10));
    fd.append("notes", fields.notesTxt);
    fd.append("codeSolution", fields.code);
    if (file) fd.append("notesImage", file);
    setQuickNoteSaving(true);
    try {
      const url = existingNote ? `/api/notes/leetcode/${existingNote.id}` : "/api/notes/leetcode";
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/notes/leetcode"] });
      if (!chapterSolvedProblems[num]) toggleChapterProblemSolved(num);
      setActiveProblemNote(null);
      quickNoteFileRef.current[num] = null;
      toast({ title: `LC-${num} saved ✓`, description: "Note added to your Notes & Problems collection." });
    } catch (e: any) {
  ...
  const saveChapterEdit = () => {
    ...
    const updated = { ...chapterOverrides, [selectedChapter.id]: { name: editChapterName, overview: editChapterOverview, notes: editChapterNotes } };
    setChapterOverrides(updated);
    localStorage.setItem("chapter-overrides", JSON.stringify(updated));
    setSelectedChapter({ ...selectedChapter, name: editChapterName, overview: editChapterOverview });
  ...
  const sigmaFilteredCategories = useMemo(() => {
    const q = sigmaSearch.trim().toLowerCase();
    if (!q) return sigmaActiveTrack.categories;
    return sigmaActiveTrack.categories
      .map((cat) => ({
        ...cat,
  ...
  // Study Dates & Contest Hyper-Calendar State
  const [plannerPace, setPlannerPace] = useState(() => localStorage.getItem("planner_pace") || "6 Days / Wk");
  const [plannerStartDate, setPlannerStartDate] = useState(() => localStorage.getItem("planner_start_date") || "2026-06-27");
  const [plannerTargetDate, setPlannerTargetDate] = useState(() => localStorage.getItem("planner_target_date") || "2026-09-18");
  const [isEditDatesOpen, setIsEditDatesOpen] = useState(false);
  const [contestPlatformFilter, setContestPlatformFilter] = useState<string>("ALL");
  const [contestSearchQuery, setContestSearchQuery] = useState<string>("");
  const [calendarThemeMode, setCalendarThemeMode] = useState<"LIGHT" | "DARK">(() => (localStorage.getItem("calendar_theme_mode") as any) || "LIGHT");

  // HUD Matrix Cards State
  const [hudSector, setHudSector] = useState(() => localStorage.getItem("custom_hud_sector") || "Phase 01: Core Architecture");
  const [hudSkillPayload, setHudSkillPayload] = useState(() => localStorage.getItem("custom_hud_payload") || "Unlocks dynamic UI state engine & reactive DOM tree node synchronization.");
  const [hudTacticalGateway, setHudTacticalGateway] = useState(() => localStorage.getItem("custom_hud_gateway") || "Singly & Doubly LL");
  const [isEditHudCardsOpen, setIsEditHudCardsOpen] = useState(false);

  useEffect(() => {
    const handleHudUpdate = () => {
      setHudSector(localStorage.getItem("custom_hud_sector") || "Phase 01: Core Architecture");
      setHudSkillPayload(localStorage.getItem("custom_hud_payload") || "Unlocks dynamic UI state engine & reactive DOM tree node synchronization.");
      setHudTacticalGateway(localStorage.getItem("custom_hud_gateway") || "Singly & Doubly LL");
      setPlannerPace(localStorage.getItem("planner_pace") || "6 Days / Wk");
      setPlannerStartDate(localStorage.getItem("planner_start_date") || "2026-06-27");
      setPlannerTargetDate(localStorage.getItem("planner_target_date") || "2026-09-18");
    };
    window.addEventListener("storage", handleHudUpdate);
    window.addEventListener("hud-settings-changed", handleHudUpdate);
    return () => {
      window.removeEventListener("storage", handleHudUpdate);
      window.removeEventListener("hud-settings-changed", handleHudUpdate);
    };
  }, []);
  ...
  const calendarSchedule: CalendarTask[] = useMemo(() => {
    ...
    const baseParts = plannerStartDate.split("-").map(Number);
    let currentDate = baseParts.length === 3 ? new Date(baseParts[0], baseParts[1] - 1, baseParts[2]) : new Date(START_DATE);
    let mernIdx = 0;
    let javaIdx = 0;
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-24 text-white">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center p-1.5 rounded-2xl bg-black/50 border border-white/15 backdrop-blur-md">
            ...
            <button onClick={() => setViewMode("CALENDAR")} className={`px-3.5 py-2 rounded-xl text-xs font-black tracking-wider uppercase transition-all flex items-center gap-1.5 ${viewMode === "CALENDAR" ? "bg-gradient-to-r from-[#90EE90] to-[#AFEEEE] text-[#1a1a24] shadow-[0_0_15px_rgba(144,238,144,0.4)]" : "text-white/40 hover:text-white/80"}`}>
              <CalendarIcon size={14} /> Contest Calendar
            </button>
            <button onClick={() => setViewMode("ACADEMICS")} className={`px-3.5 py-2 rounded-xl text-xs font-black tracking-wider uppercase transition-all flex items-center gap-1.5 ${viewMode === "ACADEMICS" ? "bg-gradient-to-r from-purple-400 to-indigo-400 text-[#1a1a24] shadow-[0_0_15px_rgba(168,85,247,0.4)]" : "text-white/40 hover:text-white/80"}`}>
              <BookOpen size={14} /> Academic Calendar
            </button>
          ...
          <TiltPanel glowColor="rgba(255,182,193,0.35)">
            <div className="rounded-3xl p-6 lg:p-8 relative overflow-hidden" style={GLASS_CARD}>
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/10">
                <span className="text-xs font-black tracking-widest uppercase text-white/40">HUD Mission Command Telemetry Matrix</span>
                <Dialog open={isEditHudCardsOpen} onOpenChange={setIsEditHudCardsOpen}>
                  <DialogTrigger asChild>
                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-white/10 hover:bg-white/15 border border-white/20 text-white transition-all">
                      <PenLine size={12} /> Customize HUD Matrix Cards
                    </button>
                  </DialogTrigger>
                  <DialogContent className="sm:max-w-[425px] bg-[#1a1a24] text-white border-white/10">
                    <DialogHeader><DialogTitle>Customize HUD Telemetry Cards</DialogTitle></DialogHeader>
                    <form onSubmit={(e) => {
                      e.preventDefault();
                      localStorage.setItem("custom_hud_sector", hudSector);
                      localStorage.setItem("custom_hud_payload", hudSkillPayload);
                      localStorage.setItem("custom_hud_gateway", hudTacticalGateway);
                      setIsEditHudCardsOpen(false);
                      toast({ title: "HUD Telemetry Cards Updated! ⚡" });
                    }} className="space-y-4 mt-2">
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-white/70">01. Current Sector Title</label>
                        <input
                          type="text"
                          value={hudSector}
                          onChange={(e) => setHudSector(e.target.value)}
                          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white"
                          placeholder="e.g. Phase 01: Core Architecture"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-white/70">03. Skill Payload Description</label>
                        <textarea
                          value={hudSkillPayload}
                          onChange={(e) => setHudSkillPayload(e.target.value)}
                          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white h-20"
                          placeholder="Skill payload overview..."
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-medium text-white/70">04. Tactical Gateway Title</label>
                        <input
                          type="text"
                          value={hudTacticalGateway}
                          onChange={(e) => setHudTacticalGateway(e.target.value)}
                          className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white"
                          placeholder="e.g. Singly & Doubly LL"
                        />
                      </div>
                      <button type="submit" className="w-full py-2.5 rounded-xl font-bold bg-gradient-to-r from-[#FFB6C1] to-[#C8A2C8] text-black">Save Matrix Cards</button>
                    </form>
                  </DialogContent>
                </Dialog>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 relative z-10">
                <div className="border-b lg:border-b-0 lg:border-r border-white/10 pb-4 lg:pb-0 lg:pr-6">
                  ...
                  <div className="text-xl font-black text-white mb-1">{hudSector}</div>
                ...
                <div className="border-b lg:border-b-0 lg:border-r border-white/10 pb-4 lg:pb-0 lg:pr-6">
                  ...
                  <div className="text-sm font-bold text-white/90 line-clamp-2">{hudSkillPayload}</div>
                </div>
                <div>
                  ...
                  <div className="text-xl font-black text-[#AFEEEE] truncate">{hudTacticalGateway}</div>
      ...
      {/* ── VIEW 2: 20,000x BETTER CONTEST & ROADMAP HYPER-CALENDAR MATRIX ── */}
      {viewMode === "CALENDAR" && (
        <div className="space-y-8">
          {/* Top Control Header Bar */}
          <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-3xl border transition-all ${calendarThemeMode === "LIGHT" ? "bg-white/95 text-slate-900 border-slate-200 shadow-xl" : "bg-white/5 text-white border-white/10 backdrop-blur-xl"}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${calendarThemeMode === "LIGHT" ? "bg-sky-100 text-sky-600 border border-sky-300" : "bg-sky-500/20 text-sky-300 border border-sky-500/40"}`}>
                <CalendarIcon size={20} />
              </div>
              <div>
                <h2 className={`text-xl font-black flex items-center gap-2 ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>
                  Live Contest Hyper-Calendar & Schedule Matrix
                </h2>
                <p className={`text-xs ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/50"}`}>Multi-platform coding contests tracker with 1-click Google Calendar sync & pace controls.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="relative w-44">
                <Search size={14} className={`absolute left-3 top-2.5 ${calendarThemeMode === "LIGHT" ? "text-slate-400" : "text-white/40"}`} />
                <input
                  type="text"
                  placeholder="Search contests..."
                  value={contestSearchQuery}
                  onChange={(e) => setContestSearchQuery(e.target.value)}
                  className={`w-full border rounded-xl pl-9 pr-3 py-1.5 text-xs focus:outline-none ${calendarThemeMode === "LIGHT" ? "bg-slate-50 border-slate-300 text-slate-800 focus:border-sky-500" : "bg-white/5 border-white/10 text-white focus:border-sky-400"}`}
                />
              </div>

              {/* Platform Selector */}
              <div className={`flex p-1 rounded-xl border text-[11px] ${calendarThemeMode === "LIGHT" ? "bg-slate-100 border-slate-200" : "bg-white/5 border-white/10"}`}>
                {["ALL", "LeetCode", "Codeforces", "CodeChef", "AtCoder", "GFG"].map((plt) => (
                  <button
                    key={plt}
                    onClick={() => setContestPlatformFilter(plt)}
                    className={`px-2.5 py-1 rounded-lg font-bold transition-all ${contestPlatformFilter === plt ? "bg-sky-500 text-white shadow" : calendarThemeMode === "LIGHT" ? "text-slate-600 hover:text-slate-900" : "text-white/50 hover:text-white"}`}
                  >
                    {plt}
                  </button>
                ))}
              ...
              <button
                onClick={() => {
                  const next = calendarThemeMode === "LIGHT" ? "DARK" : "LIGHT";
                  setCalendarThemeMode(next);
                  localStorage.setItem("calendar_theme_mode", next);
                }}
                className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all flex items-center gap-1.5 ${calendarThemeMode === "LIGHT" ? "bg-slate-100 border-slate-300 text-slate-800 hover:bg-slate-200" : "bg-white/10 border-white/20 text-amber-300 hover:bg-white/15"}`}
              >
                {calendarThemeMode === "LIGHT" ? "🌙 Dark Mode" : "☀️ Light Mode"}
              </button>

              <Dialog open={isEditDatesOpen} onOpenChange={setIsEditDatesOpen}>
                <DialogTrigger asChild>
                  <button className="px-4 py-2 rounded-xl text-xs font-black bg-gradient-to-r from-[#FFB6C1] to-[#C8A2C8] text-black flex items-center gap-1.5 shadow-[0_0_15px_rgba(255,182,193,0.3)] transition-transform active:scale-95">
                    <PenLine size={14} /> Customize Dates & Pace
                  </button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[425px] bg-[#1a1a24] text-white border-white/10">
                  <DialogHeader><DialogTitle>Customize Schedule & Target Dates</DialogTitle></DialogHeader>
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    localStorage.setItem("planner_pace", plannerPace);
                    localStorage.setItem("planner_start_date", plannerStartDate);
                    localStorage.setItem("planner_target_date", plannerTargetDate);
                    setIsEditDatesOpen(false);
                    toast({ title: "Roadmap Schedule & Dates Updated! 📅" });
                  }} className="space-y-4 mt-2">
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-white/70">Study Pace Frequency</label>
                      <select
                        value={plannerPace}
                        onChange={(e) => setPlannerPace(e.target.value)}
                        className="w-full bg-[#20202d] border border-white/10 rounded-lg px-3 py-2 text-sm text-white"
                      >
                        <option value="5 Days / Wk">5 Days / Wk</option>
                        <option value="6 Days / Wk">6 Days / Wk</option>
                        <option value="7 Days / Wk (Hardcore)">7 Days / Wk (Hardcore)</option>
                      </select>
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-white/70">Roadmap Start Date</label>
                      <input
                        type="date"
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-white/70">Target Completion Date</label>
                      <input
                        type="date"
                    ...
                    <button type="submit" className="w-full py-2.5 rounded-xl font-bold bg-gradient-to-r from-[#FFB6C1] to-[#C8A2C8] text-black">Save Roadmap Schedule</button>
                  </form>
                </DialogContent>
              </Dialog>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            {/* Left Sidebar: Upcoming Contests Timeline Feed */}
            <div className={`space-y-4 rounded-3xl p-6 border transition-all ${calendarThemeMode === "LIGHT" ? "bg-white/95 text-slate-900 border-slate-200 shadow-xl" : "bg-black/40 text-white border-white/10 backdrop-blur-xl"}`}>
              <div className={`border-b pb-3 ${calendarThemeMode === "LIGHT" ? "border-slate-200" : "border-white/10"}`}>
                <h3 className={`text-base font-black flex items-center gap-2 ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>
                  <Flame size={18} className="text-amber-500" /> Upcoming Contests
                </h3>
                <p className={`text-[11px] ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/40"}`}>Don't miss scheduled live events</p>
              </div>

              <div className="space-y-4 max-h-[620px] overflow-y-auto pr-1">
                {[
                  { title: "AtCoder Regular Contest 223", plt: "AtCoder", dateStr: "28-06-2026", time: "5:30 PM - 7:30 PM", color: "#0284C7" },
                  { title: "Codeforces Round 1106 (Div. 2)", plt: "Codeforces", dateStr: "28-06-2026", time: "8:05 PM - 10:05 PM", color: "#D97706" },
                  { title: "Monday Munch - DSA Challenge 009", plt: "GFG", dateStr: "29-06-2026", time: "7:00 PM - 10:00 PM", color: "#059669" },
                  { title: "Codeforces Round (Div. 3)", plt: "Codeforces", dateStr: "30-06-2026", time: "8:05 PM - 10:20 PM", color: "#D97706" },
                  { title: "Starters 245", plt: "CodeChef", dateStr: "01-07-2026", time: "8:00 PM - 10:00 PM", color: "#DC2626" },
                  { title: "AtCoder Beginner Contest 465", plt: "AtCoder", dateStr: "04-07-2026", time: "5:30 PM - 7:10 PM", color: "#0284C7" },
                  { title: "LeetCode Weekly Contest 405", plt: "LeetCode", dateStr: "05-07-2026", time: "8:00 AM - 9:30 AM", color: "#EA580C" },
                ].filter((c) => (contestPlatformFilter === "ALL" || c.plt === contestPlatformFilter) && c.title.toLowerCase().includes(contestSearchQuery.toLowerCase())).map((c, i) => (
                  <div key={i} className={`p-4 rounded-2xl border space-y-2 transition-all ${calendarThemeMode === "LIGHT" ? "bg-slate-50 border-slate-200 hover:bg-slate-100" : "bg-white/5 border-white/10 hover:bg-white/10"}`}>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-md" style={{ backgroundColor: `${c.color}15`, color: c.color, border: `1px solid ${c.color}30` }}>{c.plt}</span>
                      <span className={`text-[11px] font-mono ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/50"}`}>{c.dateStr}</span>
                    </div>
                    <div className={`text-sm font-bold leading-snug ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>{c.title}</div>
                    <div className="flex items-center justify-between pt-1">
                      <span className={`text-[11px] flex items-center gap-1 ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/50"}`}><Clock size={12} /> {c.time}</span>
                      <button
                        onClick={() => {
                          const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(c.title)}&details=${encodeURIComponent(`Live contest on ${c.plt}`)}`;
            ...
            {/* Right Area: Full 7-Day Monthly Grid (June 2026) */}
            <div className={`lg:col-span-2 space-y-4 rounded-3xl p-6 border transition-all ${calendarThemeMode === "LIGHT" ? "bg-white/95 text-slate-900 border-slate-200 shadow-xl" : "bg-black/40 text-white border-white/10 backdrop-blur-xl"}`}>
              <div className={`flex items-center justify-between border-b pb-3 ${calendarThemeMode === "LIGHT" ? "border-slate-200" : "border-white/10"}`}>
                <h3 className={`text-lg font-black ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>June 2026</h3>
                <div className="flex items-center gap-2">
                  <button className={`p-1.5 rounded-xl border ${calendarThemeMode === "LIGHT" ? "bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200" : "bg-white/5 border-white/10 text-white/60 hover:text-white"}`}><ChevronLeft size={16} /></button>
                  <button className={`p-1.5 rounded-xl border ${calendarThemeMode === "LIGHT" ? "bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200" : "bg-white/5 border-white/10 text-white/60 hover:text-white"}`}><ChevronRight size={16} /></button>
                </div>
              </div>

              {/* Day Name Header Row */}
              <div className={`grid grid-cols-7 gap-2 text-center text-xs font-bold uppercase tracking-wider py-2 ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/40"}`}>
                <div>Sun</div><div>Mon</div><div>Tue</div><div>Wed</div><div>Thu</div><div>Fri</div><div>Sat</div>
              </div>

              {/* 35 Calendar Cells Matrix */}
              <div className="grid grid-cols-7 gap-2">
                {[
                  { day: 31, isOther: true }, { day: 1 }, { day: 2 }, { day: 3, contest: "Starters 241" }, { day: 4 }, { day: 5 }, { day: 6, contest: "Weekend Dev" },
                  { day: 7, contest: "Weekly Contest" }, { day: 8, contest: "Monday Munch" }, { day: 9, contest: "Educational CF" }, { day: 10, contest: "Starters 242" }, { day: 11 }, { day: 12, contest: "Codeforces Rd" }, { day: 13, contest: "CodeQUEEN" },
                  { day: 14, contest: "Weekend Dev" }, { day: 15, contest: "Monday Munch" }, { day: 16 }, { day: 17, contest: "Starters 243" }, { day: 18 }, { day: 19 }, { day: 20, contest: "Biweekly Contest" },
                  { day: 21, contest: "Weekly Contest" }, { day: 22, contest: "Monday Munch" }, { day: 23 }, { day: 24, contest: "Starters 244" }, { day: 25 }, { day: 26 }, { day: 27, contest: "Codeforces Rd" },
                  { day: 28, contest: "ARC 223 / CF 1106", isToday: true }, { day: 29, contest: "Monday Munch" }, { day: 30, contest: "CF Div 3" }, { day: 1, isOther: true, contest: "Starters 245" }, { day: 2, isOther: true }, { day: 3, isOther: true }, { day: 4, isOther: true, contest: "AtCoder 465" }
                ].map((cell, idx) => (
                  <div
                    key={idx}
                    className={`min-h-[85px] p-2 rounded-xl border flex flex-col justify-between transition-all ${
                      cell.isToday
                        ? calendarThemeMode === "LIGHT" ? "bg-sky-50 border-sky-400 shadow-md ring-2 ring-sky-300" : "bg-sky-500/10 border-sky-500/50 shadow-[0_0_15px_rgba(56,189,248,0.2)]"
                        : cell.isOther
                        ? calendarThemeMode === "LIGHT" ? "bg-slate-100/50 border-slate-200/50 opacity-40 text-slate-400" : "bg-white/2 border-white/5 opacity-40"
                        : calendarThemeMode === "LIGHT" ? "bg-white border-slate-200 hover:border-slate-300 shadow-sm" : "bg-white/5 border-white/10 hover:border-white/20"
                    }`}
                  >
                    <div className="flex justify-end">
                      <span className={`text-xs font-bold ${cell.isToday ? "text-sky-700 bg-sky-100 px-1.5 py-0.5 rounded-md" : calendarThemeMode === "LIGHT" ? "text-slate-700" : "text-white/60"}`}>{cell.day}</span>
                    </div>
                    {cell.contest && (
                      <div className="mt-1">
                        <span className={`text-[9px] font-bold block truncate px-1.5 py-0.5 rounded border ${calendarThemeMode === "LIGHT" ? "bg-amber-100 text-amber-900 border-amber-300 font-extrabold" : "bg-amber-500/20 text-amber-300 border-amber-500/30"}`} title={cell.contest}>
                          {cell.contest}
                        </span>
                      </div>
                    )}
      ...
      {/* ── VIEW: 20,000x ACADEMIC EXAMS & MILESTONES HYPER-CALENDAR MATRIX ── */}
      {viewMode === "ACADEMICS" && (
        <div className="space-y-8">
          {/* Top Control Header Bar */}
          <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-3xl border transition-all ${calendarThemeMode === "LIGHT" ? "bg-white/95 text-slate-900 border-purple-200 shadow-xl" : "border-purple-500/30 bg-gradient-to-r from-purple-950/40 via-background to-indigo-950/40 backdrop-blur-xl"}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${calendarThemeMode === "LIGHT" ? "bg-purple-100 text-purple-600 border border-purple-300" : "bg-purple-500/20 text-purple-300 border border-purple-500/40"}`}>
                <BookOpen size={20} />
              </div>
              <div>
                <h2 className={`text-xl font-black flex items-center gap-2 ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>
                  Academic Exams & Semester Schedule Matrix
                </h2>
                <p className={`text-xs ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/50"}`}>Track RGPV university forms, midterm exams, lab vivas, and project submissions.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent("RGPV Semester Exam Form & Vivas")}`;
                  window.open(url, "_blank");
                }}
                className="px-4 py-2 rounded-xl text-xs font-black bg-gradient-to-r from-purple-500 to-indigo-500 text-white flex items-center gap-1.5 shadow-[0_0_15px_rgba(168,85,247,0.3)] transition-transform active:scale-95"
              >
                <ExternalLink size={14} /> Sync Academic Milestones to iCal/Google
              </button>
            </div>
          </div>
          ...
          {/* Master 2-Column Grid */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            {/* Left Sidebar: Academic Milestones Timeline Feed */}
            <div className={`space-y-4 rounded-3xl p-6 border transition-all ${calendarThemeMode === "LIGHT" ? "bg-white/95 text-slate-900 border-slate-200 shadow-xl" : "bg-black/40 text-white border-white/10 backdrop-blur-xl"}`}>
              <div className={`border-b pb-3 ${calendarThemeMode === "LIGHT" ? "border-slate-200" : "border-white/10"}`}>
                <h3 className={`text-base font-black flex items-center gap-2 ${calendarThemeMode === "LIGHT" ? "text-purple-700" : "text-purple-400"}`}>
                  <Award size={18} /> Key Academic Deadlines
                </h3>
                <p className={`text-[11px] ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/40"}`}>Exams, vivas & submission milestones</p>
              </div>

              <div className="space-y-4 max-h-[620px] overflow-y-auto pr-1">
                {[
                  { title: "Project Print Submission", category: "Coursework", dateStr: "03-12-2026", time: "11:00 AM", color: "#DB2777" },
                  { title: "Yt Premium & Recharges", category: "Bills", dateStr: "05-12-2026", time: "5:00 PM", color: "#0284C7" },
                  { title: "Exam RGPV Form Deadline", category: "University", dateStr: "09-12-2026", time: "11:59 PM", color: "#DC2626" },
                  { title: "Operating Systems Mid-Sem Viva", category: "Lab Viva", dateStr: "15-12-2026", time: "10:00 AM", color: "#D97706" },
                  { title: "DBMS Theory Examination", category: "Final Exam", dateStr: "22-12-2026", time: "2:00 PM", color: "#9333EA" },
                  { title: "Computer Networks Seminar", category: "Presentation", dateStr: "28-12-2026", time: "11:30 AM", color: "#059669" },
                ].map((item, i) => (
                  <div key={i} className={`p-4 rounded-2xl border space-y-2 transition-all ${calendarThemeMode === "LIGHT" ? "bg-slate-50 border-slate-200 hover:bg-slate-100" : "bg-white/5 border-white/10 hover:bg-white/10"}`}>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-md" style={{ backgroundColor: `${item.color}15`, color: item.color, border: `1px solid ${item.color}30` }}>{item.category}</span>
                      <span className={`text-[11px] font-mono ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/50"}`}>{item.dateStr}</span>
                    </div>
                    <div className={`text-sm font-bold leading-snug ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>{item.title}</div>
                    <div className="flex items-center justify-between pt-1">
                      <span className={`text-[11px] flex items-center gap-1 ${calendarThemeMode === "LIGHT" ? "text-slate-500" : "text-white/50"}`}><Clock size={12} /> {item.time}</span>
                      <button
                        onClick={() => {
                          const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(item.title)}`;
                          window.open(url, "_blank");
                        }}
            ...
            {/* Right Area: Full 7-Day Monthly Grid (December 2026 / Current Term) */}
            <div className={`lg:col-span-2 space-y-4 rounded-3xl p-6 border transition-all ${calendarThemeMode === "LIGHT" ? "bg-white/95 text-slate-900 border-slate-200 shadow-xl" : "bg-black/40 text-white border-white/10 backdrop-blur-xl"}`}>
              <div className={`flex items-center justify-between border-b pb-3 ${calendarThemeMode === "LIGHT" ? "border-slate-200" : "border-white/10"}`}>
                <h3 className={`text-lg font-black ${calendarThemeMode === "LIGHT" ? "text-slate-900" : "text-white"}`}>December 2026 (Semester Exam Term)</h3>
                <div className="flex items-center gap-2">
                  <button className={`p-1.5 rounded-xl border ${calendarThemeMode === "LIGHT" ? "bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200" : "bg-white/5 border-white/10 text-white/60 hover:text-white"}`}><ChevronLeft size={16} /></button>
                  <button className={`p-1.5 rounded-xl border ${calendarThemeMode === "LIGHT" ? "bg-slate-100 border-slate-200 text-slate-700 hover:bg-slate-200" : "bg-white/5 border-white/10 text-white/60 hover:text-white"}`}><ChevronRight size={16} /></button>
                </div>
              ...
              <div className="grid grid-cols-7 gap-2">
                {[
                  { day: 29, isOther: true }, { day: 30, isOther: true }, { day: 1 }, { day: 2 }, { day: 3, milestone: "Project Print" }, { day: 4 }, { day: 5, milestone: "YT Recharge" },
                  { day: 6 }, { day: 7 }, { day: 8 }, { day: 9, milestone: "RGPV Form", isUrgent: true }, { day: 10 }, { day: 11 }, { day: 12 },
                  { day: 13 }, { day: 14 }, { day: 15, milestone: "OS Viva" }, { day: 16 }, { day: 17 }, { day: 18 }, { day: 19 },
                  { day: 20 }, { day: 21 }, { day: 22, milestone: "DBMS Exam" }, { day: 23 }, { day: 24 }, { day: 25 }, { day: 26 },
                  { day: 27 }, { day: 28, milestone: "CN Seminar" }, { day: 30 }, { day: 31 }, { day: 1, isOther: true }, { day: 2, isOther: true }
                ].map((cell, idx) => (
                  <div
                    key={idx}
                    className={`min-h-[85px] p-2 rounded-xl border flex flex-col justify-between transition-all ${
                      cell.isUrgent
                        ? calendarThemeMode === "LIGHT" ? "bg-rose-50 border-rose-300 shadow" : "bg-rose-500/10 border-rose-500/50 shadow-[0_0_15px_rgba(244,63,94,0.2)]"
                        : cell.isOther
                        ? calendarThemeMode === "LIGHT" ? "bg-slate-100/50 border-slate-200/50 opacity-40 text-slate-400" : "bg-white/2 border-white/5 opacity-40"
                        : calendarThemeMode === "LIGHT" ? "bg-white border-slate-200 hover:border-slate-300 shadow-sm" : "bg-white/5 border-white/10 hover:border-white/20"
                    }`}
                  >
                    <div className="flex justify-end">
                      <span className={`text-xs font-bold ${calendarThemeMode === "LIGHT" ? "text-slate-700" : "text-white/60"}`}>{cell.day}</span>
                    </div>
                    {cell.milestone && (
                      <div className="mt-1">
                        <span className={`text-[9px] font-bold block truncate px-1.5 py-0.5 rounded border ${
                          cell.isUrgent
                            ? calendarThemeMode === "LIGHT" ? "bg-rose-100 text-rose-900 border-rose-300 font-extrabold" : "bg-rose-500/20 text-rose-300 border-rose-500/40"
                            : calendarThemeMode === "LIGHT" ? "bg-purple-100 text-purple-900 border-purple-300 font-extrabold" : "bg-purple-500/20 text-purple-300 border-purple-500/30"
        ...
        <div className="space-y-5">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              value={sigmaSearch}
              onChange={(e) => { setSigmaSearch(e.target.value); setExpandedSigmaCategory(null); }}
              placeholder="Search chapters or problem numbers (e.g. LC-70, Binary Search)…"
            ...
            />
            {sigmaSearch && (
              <button onClick={() => setSigmaSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/70 transition-colors">
                <X size={14} />
              </button>
            )}
          </div>
          ...
          <div className="space-y-3">
            {sigmaSearch.trim() && sigmaFilteredCategories.length === 0 && (
              <div className="py-12 text-center text-white/30 text-sm">
                No chapters match <span className="font-bold text-white/50">"{sigmaSearch}"</span>
              </div>
            )}
            {sigmaFilteredCategories.map((cat) => {
                        ...
                        <div className="border-t border-white/05 px-5 pb-4 pt-2 space-y-1">
                          {cat.chapters.map((ch) => {
                            ...
                            return (
                              <div
                                key={ch.id}
                                onClick={() => { const ov = chapterOverrides[ch.id]; setSelectedChapter({ id: ch.id, name: ov?.name ?? ch.name, overview: ov?.overview ?? ch.overview ?? "", trackColor: sigmaActiveTrack.color }); setIsEditingChapter(false); }}
              ...
              >
                ...
                <div className="flex items-start gap-4 p-6 border-b border-white/08" style={{ borderBottomColor: `${color}22` }}>
                    ...
                    {isEditingChapter ? (
                      <div className="space-y-2">
                        <input
                          ...
                          className="w-full bg-white/8 border rounded-lg px-3 py-1.5 text-sm font-black text-white focus:outline-none"
                          style={{ borderColor: `${color}55` }}
                          placeholder="Chapter name"
                        />
                        <textarea
                          value={editChapterOverview}
                          onChange={(e) => setEditChapterOverview(e.target.value)}
                          rows={3}
                          className="w-full bg-white/8 border rounded-lg px-3 py-1.5 text-xs text-white/70 focus:outline-none resize-none leading-relaxed"
                          style={{ borderColor: `${color}55` }}
                          placeholder="Overview / problem list..."
                        />
                        <textarea
                          ...
                          className="w-full bg-white/8 border rounded-lg px-3 py-1.5 text-xs text-white/70 focus:outline-none resize-none leading-relaxed"
                          style={{ borderColor: `${color}55` }}
                          placeholder="Your personal notes, study tips, key insights..."
                        />
                        <div className="flex items-center gap-2 pt-1">
                          <button
                            onClick={saveChapterEdit}
                            className="px-3 py-1 rounded-lg text-xs font-bold transition-all"
                            style={{ background: color, color: "#1a1a24" }}
                          >
                            Save
                          </button>
                          <button
                            onClick={() => setIsEditingChapter(false)}
                            className="px-3 py-1 rounded-lg text-xs font-semibold text-white/50 hover:text-white/80 transition-all"
                          >
                            Cancel
                          </button>
                        </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {!isEditingChapter && (
                      <button
                        onClick={() => { setEditChapterName(selectedChapter.name); setEditChapterOverview(selectedChapter.overview); setEditChapterNotes(chapterOverrides[selectedChapter.id]?.notes ?? ""); setIsEditingChapter(true); }}
                        className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/10 text-white/30 hover:text-white transition-all"
                        title="Edit chapter"
                      >
                        <Edit3 size={14} />
                      </button>
                    )}
                    <button onClick={() => { setSelectedChapter(null); setIsEditingChapter(false); }} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/10 text-white/40 hover:text-white flex-shrink-0 transition-all">
                      <X size={16} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {lcNums.map((num) => {
                          ...
                          const isNoteOpen = activeProblemNote === num;
                          const fields = quickNoteFields[num] ?? { title: note?.title ?? `LC-${num}`, difficulty: note?.difficulty ?? "Medium", notesTxt: note?.notes ?? "", code: note?.codeSolution ?? "" };

                          return (
                            <div key={num} className="rounded-xl overflow-hidden" style={{ border: `1px solid ${isNoteOpen ? color + "40" : solved ? color + "28" : "rgba(255,255,255,0.06)"}`, transition: "border-color 0.2s" }}>
                              {/* Problem row */}
                              <div className="flex items-center gap-3 px-3 py-2.5 transition-all" style={{ background: isNoteOpen ? `${color}0f` : solved ? `${color}09` : "rgba(255,255,255,0.03)" }}>
                                <button
                                ...
                                {/* Note button */}
                                <button
                                  onClick={() => openQuickNote(num, note)}
                                  title={note ? "Edit saved note" : "Add note for this problem"}
                                  className="flex-shrink-0 flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold transition-all hover:scale-105"
                                  style={{
                                    background: note ? `${color}22` : isNoteOpen ? `${color}18` : "rgba(255,255,255,0.05)",
                                    border: `1px solid ${note ? color + "40" : isNoteOpen ? color + "30" : "rgba(255,255,255,0.08)"}`,
                                    color: note ? color : isNoteOpen ? color : "rgba(255,255,255,0.3)",
                                  }}
                                >
                                  {isNoteOpen ? <ChevronUp size={10} /> : <PenLine size={10} />}
                              ...
                              <AnimatePresence>
                                {isNoteOpen && (
                                  ...
                                  >
                                    <div className="px-3 pb-3 pt-1 space-y-2.5" style={{ borderTop: `1px solid ${color}20`, background: `${color}06` }}>
                                      {/* Title + Difficulty row */}
                                      <div className="flex gap-2">
                                        <div className="flex-1">
                                          <div className="text-[9px] font-bold uppercase tracking-wider mb-1 text-white/30">Title</div>
                                          <input
                                            className="w-full text-xs rounded-lg px-2.5 py-1.5 outline-none focus:ring-1"
                                            style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.85)" }}
                                            placeholder={`LC-${num} title`}
                                            value={fields.title}
                                            onChange={(e) => patchQuickNote(num, { title: e.target.value })}
                                          />
                                        </div>
                                        <div className="w-24">
                                          <div className="text-[9px] font-bold uppercase tracking-wider mb-1 text-white/30">Difficulty</div>
                                          <select
                                            ...
                                            onChange={(e) => patchQuickNote(num, { difficulty: e.target.value })}
                                          >
                                            <option>Easy</option>
                                      ...
                                      {/* Notes textarea */}
                                      <div>
                                        <div className="text-[9px] font-bold uppercase tracking-wider mb-1 text-white/30">Notes (approach, complexity, insights)</div>
                                        <textarea
                                          className="w-full text-xs rounded-lg px-2.5 py-1.5 outline-none resize-none"
                                          style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", color: "rgba(255,255,255,0.75)", minHeight: 64 }}
                                          placeholder="Key observations, approach taken, time/space complexity..."
                                          rows={3}
                                          value={fields.notesTxt}
                                          onChange={(e) => patchQuickNote(num, { notesTxt: e.target.value })}
                                        />
                                      </div>

                                      {/* Code textarea */}
                                      <div>
                                        <div className="text-[9px] font-bold uppercase tracking-wider mb-1 text-white/30">Code Solution</div>
                                        <textarea
                                          className="w-full text-xs rounded-lg px-2.5 py-1.5 outline-none resize-none"
                                          style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", color: "rgba(255,255,255,0.7)", fontFamily: "'Fira Code', monospace", minHeight: 72 }}
                                          placeholder="Paste your solution code here..."
                                          rows={4}
                                          value={fields.code}
                                          onChange={(e) => patchQuickNote(num, { code: e.target.value })}
                                        />
                                      </div>

                                      {/* File upload */}
                                      <div>
                                        <div className="text-[9px] font-bold uppercase tracking-wider mb-1 text-white/30">Upload Notes PDF / Photo</div>
                                        {quickNoteFileRef.current[num] ? (
                                          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs" style={{ background: `${color}12`, border: `1px solid ${color}30` }}>
                                            <FileText size={11} style={{ color }} />
                                            <span className="flex-1 truncate text-white/60">{quickNoteFileRef.current[num]!.name}</span>
                                            <button
                                              type="button"
                                              onClick={() => { quickNoteFileRef.current[num] = null; setQuickNoteFields((p) => ({ ...p })); }}
                                              className="text-white/30 hover:text-white/70"
                                            ><X size={10} /></button>
                                          </div>
                                        ) : (
                                          <label className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs cursor-pointer hover:bg-white/5 transition-colors"
                                            style={{ background: "rgba(255,255,255,0.03)", border: "1px dashed rgba(255,255,255,0.1)" }}>
                                            <Upload size={11} className="text-white/25" />
                                            <span className="text-white/30">Click to upload (image or PDF · max 20MB)</span>
                                            {note?.notesImageUrl && (
                                              <a href={note.notesImageUrl} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="ml-auto text-[10px] underline flex-shrink-0" style={{ color }}>existing ↗</a>
                                            )}
                                            <input
                                              type="file"
                                              ...
                                              onChange={(e) => {
                                                ...
                                                if (f && f.size <= 20 * 1024 * 1024) {
                                                  quickNoteFileRef.current[num] = f;
                                                  setQuickNoteFields((p) => ({ ...p }));
                                                } else if (f) {
                                      ...
                                      <div className="flex items-center gap-2 pt-0.5">
                                        <button
                                          onClick={() => saveQuickNote(num, note)}
                                          ...
                                          style={{ background: `linear-gradient(135deg, ${color}, ${color}99)`, color: "#0a0a14" }}
                                        >
                                          <Save size={10} />
                                          {quickNoteSaving ? "Saving…" : note ? "Update Note" : "Save Note"}
                                        </button>
                                        <button
                                          onClick={() => setActiveProblemNote(null)}
                                        ...
                                        {note && (
                                          ...
                                          >
                                            View in Notes & Problems <ExternalLink size={9} />
                                          </a>
                                        )}
                                      </div>
                                    </div>
                  ...
                  {(() => {
                    const personalNotes = chapterOverrides[selectedChapter.id]?.notes;
                    return (
                      <div>
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <Edit3 size={13} style={{ color }} />
                            <span className="text-xs font-black uppercase tracking-widest" style={{ color }}>My Study Notes</span>
                          </div>
                          <button
                            onClick={() => { setEditChapterName(selectedChapter.name); setEditChapterOverview(selectedChapter.overview); setEditChapterNotes(chapterOverrides[selectedChapter.id]?.notes ?? ""); setIsEditingChapter(true); }}
                            className="text-[10px] font-semibold text-white/30 hover:text-white/70 transition-colors flex items-center gap-1"
                          >
                            <Edit3 size={10} /> {personalNotes ? "Edit" : "Add notes"}
                          </button>
                        </div>
                        {personalNotes ? (
                          <div className="rounded-xl px-4 py-3 text-xs text-white/60 leading-relaxed whitespace-pre-wrap" style={{ background: `${color}08`, border: `1px solid ${color}22` }}>
                            {personalNotes}
                          </div>
                        ) : (
                          <div className="rounded-xl px-4 py-5 text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.07)" }}>
                            <p className="text-xs text-white/25">No personal notes yet.</p>
                            <p className="text-[11px] text-white/18 mt-0.5">Click <span className="font-semibold">Add notes</span> above to jot down your thoughts.</p>
                          </div>
                        )}
                      </div>
                    );
